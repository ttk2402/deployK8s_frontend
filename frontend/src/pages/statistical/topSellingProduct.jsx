...
const TopSellingProduct = () => {
    ...
    useEffect(() => {
        const fetchOrdersData = async () => {
            try {
                const response = await axios.get('http://foodapp.k8s.com/api/product/');
                ...
                const productSalesPromises = response.data.map(async product => {
                    const salesResponse = await axios.get(`http://foodapp.k8s.com/api/order/purchases/${product.id}`);
