...
const AllTableStaffMngt = () => { //{Data}
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://foodapp.k8s.com/api/account/2');
    ...
    const handleLockUnlock = async (params) => {
        ...
        try {
            if (currentStatus === 'active') {
                const response = await axios.put(`http://foodapp.k8s.com/api/account/block/${accountId}`);
                ...
                const response = await axios.put(`http://foodapp.k8s.com/api/account/open/${accountId}`);
                if (response.status === 200) {
    ...
    const handleDelete = async (id) => {
        setShowProgressBar(true);
        try {
            const response = await axios.delete(`http://foodapp.k8s.com/api/account/${id}`, {
                headers: {
