    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                switch (type) {
                    case "product":
                        apiUrl = "http://foodapp.k8s.com/api/product/"; // Địa chỉ API để lấy tất cả sản phẩm
                        break;
                    case "order":
                        apiUrl = "http://foodapp.k8s.com/api/order/all"; // Địa chỉ API để lấy số lượng đơn hàng
                        break;
                    case "discount":
                        apiUrl = "http://foodapp.k8s.com/api/discount/all"; // Địa chỉ API để lấy số lượng đánh giá
                        break;
                    case "user":
                        apiUrl = "http://foodapp.k8s.com/api/account/1"; // Địa chỉ API để lấy số lượng khách hàng
                        break;
                    case "review":
                        apiUrl = "http://foodapp.k8s.com/api/review/all"; // Địa chỉ API để lấy số lượng đánh giá
                        break;
                    default:
                        break;
                }
